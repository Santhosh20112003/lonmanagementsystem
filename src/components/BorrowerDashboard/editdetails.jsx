...
export default function Edit() {
  ...
  const [aadharnumber,setaadharnumber] = useState(details.aadharnumber);
  ...
  const handleSubmit = (e) => {
    ...
    if (!bankDetails) {
      validationErrors.bankDetails = "bankdetails is required";
    }
    if (!dob) {
      validationErrors.dob = "Dob is required with age above 18 years.";
    }
    console.log(dob)
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      if(consent){
        axios.post("http://localhost:5000/updatedetails",{
        id:details.id,
        name:name,
                ...
                {errors.bankDetails && <p className="text-red-500 text-xs mt-1 w-60">{errors.bankDetails}</p>}
                    ...
                    <div className="mb-4">
                      ...
                      <input
                        type="number"
                        className={`${
                          errors.aadharnumber ? 'border-red-500' : 'border'
                        } shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
                        value={aadharnumber}

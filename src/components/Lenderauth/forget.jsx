...
export function LForget() {
  ...
  const handlePasswordSubmit = (e) => {
    ...
    if(!password && !confirmPassword){
      ...
      setMessage('');
        ...
        if (password.length >= strongCriteria.minLength) {
        ...
        if(currentStrength=="Password Strength: Strong" || currentStrength=="Password Strength: Moderate"){
          ...
          axios.post('http://localhost:5000/lchangepassword',formdata)
          .then((res)=>{
            if(res.status==200){
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      axios.post("http://localhost:5000/lgenerateotp", historyData)
        .then((res) => {
          if (res.status === 200) {
  ...
  const handleOTPSubmit = (e) => {
   ...
   const otpdata = {
    ...
    axios.post("http://localhost:5000/lcheckotp",otpdata)
    .then((res)=>{
      if(res.status==200){
         setOtpverified(true);
      }

...
function Navbar() {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:5000/lendergetdetails", { token: window.localStorage.getItem("tokenlender") })
      .then((res) => {
        const user = res.data;
		console.log(user);
        setDetails({
        ...
        <div className="flex justify-between h-[10vh]">
          <div className="flex items-center">
            <span className="flex md:hidden lg:hidden">
              ...
              <ul
                className={`md:flex md:items-center w-[250px]  md:pb-0 pb-8 absolute md:static md:z-auto left-0 top-0 h-screen z-[30] md:w-auto md:pl-0  md:bg-transparent bg-indigo-900 transition-all duration-500 ease-in ${
                  ...
                  >
                    <Link
                      ...
                      className={`text-white px-3  py-2 rounded-md ${
						location.pathname.includes(link.link) ? 'bg-indigo-500 text-black font-medium' : ''
					  }`}
          ...
          <div className="flex items-center">
            <div className="ml-3 relative">
              ...
              >
                <div className=" bg-pink-500 h-12 w-12 rounded-full flex items-center justify-center text-white md:text-3xl text-2xl text-center relative">
                ...
                <div className="origin-top-right z-30 absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                  ...
                  >
                    ...
                    <button
					  ...
					  onClick={(e)=>{window.localStorage.removeItem("tokenlender");window.location.href="/home"}}
                      className="block w-full text-start  px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                      role="menuitem"

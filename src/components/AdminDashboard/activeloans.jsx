  ...
  return (
    <div className="flex flex-col  h-[90vh] overflow-scroll items-center w-full">
      <div className='flex lg:w-4/5 w-full  pt-5 justify-between items-center'>
        ...
        <button className='px-2 pt-2 pb-1 rounded-md shadow-lg bg-indigo-500 text-white active:scale-110 transition-transform' onClick={toPDF}> <FontAwesomeIcon icon="fas fa-file-export" className=''></FontAwesomeIcon> Export PDF</button>
      </div>
      <div ref={targetRef} className="flex  lg:w-[95%] w-full  lg:p-0 px-2 md:px-0  h-[90vh] flex-col">
        ...
        <td colSpan="10" className='text-center py-4'>No Loans Are Taken</td>
              ...
              <tr key={loan.id} className="transition-all hover:bg-gray-100 hover:shadow-lg">
                ...
                <td className="px-2 py-4 text-sm text-gray-500  whitespace-nowrap">{loan.interest}%</td>
                <td className="px-2 py-4 text-sm text-gray-500 whitespace-nowrap">{loan.timeperiod}yrs</td>
                <td className="px-2 py-4 capitalize whitespace-nowrap"><span className={`inline-flex px-2 text-xs font-semibold leading-5 ${loan.type === 'gold' ? 'text-yellow-800 bg-yellow-100' : loan.type === 'personal' ? 'text-blue-800 bg-blue-100' : 'text-pink-800 bg-pink-100'} rounded-full`}>
                 {loan.type}
                </span>
</td>
<td className="px-2 py-4 font-bold text-sm text-black whitespace-nowrap">₹{loan.pamount}</td>
<td className="px-2 py-4 font-bold text-sm text-black whitespace-nowrap">₹{loan.total-loan.amount} </td>
                <td className="px-2 py-4 font-bold text-sm text-black whitespace-nowrap">₹{loan.amount}</td>
                <td className="px-2 py-4 font-bold text-sm text-black whitespace-nowrap">₹{loan.total} /-</td>
